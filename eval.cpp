...
static Value See(Move m) {
	...
	rooksQueens = bishopsQueens = pos.bitboard_of(WHITE_QUEEN) | pos.bitboard_of(BLACK_QUEEN);
	rooksQueens |= pos.bitboard_of(WHITE_ROOK) | pos.bitboard_of(BLACK_ROOK);
	bishopsQueens |= pos.bitboard_of(WHITE_BISHOP) | pos.bitboard_of(BLACK_BISHOP);
	Bitboard fixed = ((Shift(NORTH_WEST, sqBB) | Shift(NORTH_EAST, sqBB)) & pos.bitboard_of(BLACK_PAWN))
		| ((Shift(SOUTH_WEST, sqBB) | Shift(SOUTH_EAST, sqBB)) & pos.bitboard_of(WHITE_PAWN))
		| (PSEUDO_LEGAL_ATTACKS[KNIGHT][sqTo] & (pos.bitboard_of(WHITE_KNIGHT) | pos.bitboard_of(BLACK_KNIGHT)))
...
Value Trace(Position& pos) {
	...
	if (!esW.chance && !esB.chance)
		return VALUE_ZERO;
	Value vw = ValueMax(sw);
	Value vb = ValueMax(sb);
	if ((!esW.chance && vw >= vb) || (!esB.chance && vb >= vw))
		return VALUE_ZERO;
	Score score = sw - sb;
	if (pos.ColorBlack())
		score = -score;
...
Value ShowEval() {
	return (Trace<TRACE>(pos));
}

Value Eval() {
	return (Trace<NO_TRACE>(pos));
}